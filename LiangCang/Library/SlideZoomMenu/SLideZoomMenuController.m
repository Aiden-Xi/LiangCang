...
//    mainViewcontroller.navigationItem.leftBarButtonItem = barItem;
    ...
    // 判断传递进来的值，设置左边按钮图标
    if (self.selCount == 1) {
        [btn setBackgroundImage:[UIImage imageNamed:@"menu_back.png"] forState:UIControlStateNormal];
    } else if(self.selCount == 0) {
        [btn setBackgroundImage:[UIImage imageNamed:@"menu_button.png"] forState:UIControlStateNormal];
    }
    
    [btn addTarget:self action:@selector(showLeft) forControlEvents:UIControlEventTouchDown];
    UIBarButtonItem *barItem1 = [[UIBarButtonItem alloc] initWithCustomView:btn];
    mainViewcontroller.navigationItem.leftBarButtonItem = barItem1;
    // nav_bg
    UIImage *image = [UIImage resizableImage:@"nav_bg.png"];
    [mainViewcontroller.navigationController.navigationBar setBackgroundImage:image forBarMetrics:UIBarMetricsDefault];
}
